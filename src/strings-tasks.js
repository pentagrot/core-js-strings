...
function isPalindrome(str) {
  const tmpStr = str.replace(/[^A-Za-z0-9]/g, '').toLowerCase();
  return tmpStr === reverseString(tmpStr);
}
  ...
  return str
    ...
    .map((e) => {
      ...
      if (e.match(/[a-zA-Z]/g)) {
        ...
        let resultCharCode = e.toLowerCase().charCodeAt(0) + 13;
        if (resultCharCode >= firstLetterCode + 26) {
          resultCharCode -= 26;
        }
        result = String.fromCharCode(resultCharCode);
        result = isUpperCase ? result.toUpperCase() : result;
      }
      return result;
    })
    .join('');
...
function getCardId(value) {
  ...
  return Number.parseInt(value, 10) - 1 + offset;
}
